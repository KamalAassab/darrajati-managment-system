...
export function KPICard({ title, value, icon, trend, color = 'primary' }: KPICardProps) {
    const colorVariants = {
        primary: 'border-primary/20 text-primary [&_.icon-bg]:bg-primary/10 [&_.icon]:text-primary',
    ...
    return (
        <div className={`glass-panel rounded-2xl p-6 hover-glow transition-all duration-500 font-outfit border-white/[0.05] ${colorVariants[color]}`}>
            <div className="flex items-start justify-between">
                <div className="space-y-3">
                    ...
                    {trend && (
                        <div className={`flex items-center gap-1.5 text-[10px] font-medium uppercase tracking-widest ${trend === 'up' ? 'text-green-500' : trend === 'down' ? 'text-red-500' : 'text-primary'
                            }`}>
